	...
	fullScorecardRaw(CONFIG, user, beatmap, score) {
		...
		return new Promise((resolve, reject) => {
			this.beatmap(CONFIG, beatmap, [beatmap], { creator: beatmap.creator, creator_id: beatmap.creator_id }, getMods(score.enabled_mods).string, beatmap.mode, true).then(beatmap_embed => {
				...
				newEmbed.addField(`pp/PP${TAB}${TAB}${TAB}${TAB}${TAB}${TAB}Combo${TAB}${TAB}${TAB}${TAB}Hits`, `**${score.pp_valid ? `${score.pp}pp` : `~~${score.pp}pp~~`}**${score.max_pp_valid ? `/${score.max_pp}PP` : `${score.max_pp === 0 ? "" : `${score.max_pp.round(2)}PP`}`} ${score.maxcombo}x${beatmap.max_combo === 0 ? `/${beatmap.max_combo}` : ""}${TAB}{${beatmap.mode === 3 ? `${score.countgeki}/${score.count300}/${score.countkatu}/${score.count100}/${score.count50}/${score.countmiss}` : ` ${score.count300} / ${score.count100} / ${score.count50} / ${score.countmiss} `}}`);
